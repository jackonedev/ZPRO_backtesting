...
def moving_averages(data, label, window):
    dataux = pd.DataFrame()
    ...
    dataux[f"DEMA_{window}"] = dataux[f"EMA_{window}"] *2 - dataux[f"EMA_{window}"].ewm(span=window).mean()
    ...
    dataux[f"TEMA_{window}"] = dataux[f"EMA_{window}"] *3 - dataux[f"TRIMA_E_{window}"] *3 + dataux[f"TRIMA_E_{window}"].ewm(span=window).mean()
